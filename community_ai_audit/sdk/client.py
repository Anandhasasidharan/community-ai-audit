...
class AuditClient:
    ...
    def _request(self, method: str, path: str, body: dict | None = None) -> dict:
        ...
        req = Request(urljoin(self.base_url, path), data=data, headers=self._headers(), method=method)
        with urlopen(req, timeout=30) as resp:
            return json.loads(resp.read())

    def register(self, email: str, password: str, org_name: str = "default") -> dict:
        return self._request("POST", "/auth/register", {"email": email, "password": password, "org_name": org_name})
    ...
    def submit_audit(self, model: str, provider: str, scanners: list[str] | None = None, project_id: str | None = None) -> dict:
        if project_id:
            return self._request("POST", f"/projects/{project_id}/audits", {"model": model, "provider": provider, "scanners": scanners})
        return self._request("POST", "/audit", {"model": model, "provider": provider, "scanners": scanners})
